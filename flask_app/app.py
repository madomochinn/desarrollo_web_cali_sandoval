...
def aviso_detalle(aviso_id):
    ...
    return render_template('aviso_detalle.html', aviso=aviso, contactos=contactos, fotos=fotos, comentarios=comentarios)
...
@app.route("/get-stats-avisos-dia", methods=["GET"])
def get_stats_avisos_dia():
    """
    Obtiene datos reales de avisos por día desde la base de datos
    """
    try:
        datos = db.get_avisos_por_dia()
        return jsonify(datos)
    except Exception as e:
        print(f"ERROR en get_stats_avisos_dia: {str(e)}")
        return jsonify([])

@app.route("/get-stats-avisos-tipo", methods=["GET"])
def get_stats_avisos_tipo():
    """
    Obtiene datos reales de avisos por tipo desde la base de datos
    """
    try:
        datos = db.get_avisos_por_tipo()
        return jsonify(datos)
    except Exception as e:
        print(f"ERROR en get_stats_avisos_tipo: {str(e)}")
        return jsonify([])

@app.route("/get-stats-avisos-mes", methods=["GET"])
def get_stats_avisos_mes():
    """
    Obtiene datos reales de avisos por mes desde la base de datos
    """
    try:
        datos = db.get_avisos_por_mes()
        return jsonify(datos)
    except Exception as e:
        print(f"ERROR en get_stats_avisos_mes: {str(e)}")
        return jsonify([])

@app.route('/comentar/<int:aviso_id>', methods=['POST'])
def comentar(aviso_id):
    # obtener datos del formulario
    nombre = request.form.get('nombre')
    texto = request.form.get('texto')

    if nombre and texto and len(nombre) <= 80 and len(texto) <= 300:
        try:
            db.save_comentario(aviso_id, nombre, texto)
        except Exception as e:
            print(f"Error al guardar comentario: {e}")
    
    return redirect(url_for('aviso_detalle', aviso_id=aviso_id))
